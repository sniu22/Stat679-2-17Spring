...
3. The most costful option is to filter the data by and_mask and others. By using Rdata instead of fits files, the runtime reduces about 320 seconds.

## Code detail
1. Program the function to filter the data and save the Rdata file
...
y2 <- as_tibble(read.fitstab(file = "/u/s/h/shuoniu/679/hw3/target/shapley_lyman_break_template.fit.txt")) %>% 
  ...
  select_("FLUX","wavelength") %>% rename(flux = FLUX)
## Construct the model
model1 <- lm(flux~ poly(wavelength,4),data = y1)
model2 <- lm(flux~ poly(wavelength,4),data = y2)
...
## Construct the model
model1 <- lm(flux~ poly(wavelength,5),data = y1)
model2 <- lm(flux~ poly(wavelength,5),data = y2)
