...
load_one <- function(onepath){
  ...
  save(list = paste("d",label,sep = ""),file = paste("../hw3/rdata/",label,".Rdata",sep = ""))
}

# 2.Save the Rdata file.

## The file list
fitslist <- list.files(path = "../hw3/fits")
...
for(i in 1:length(fitslist)){
  load_one(paste("../hw3/fits/",fitslist[i],sep = ""))  
}
...
y1 <- as_tibble(read.fitstab(file = "../hw3/cb58_lyman_break_template.fit.txt")) %>% 
  mutate(wavelength = 10^LOGLAM)  %>% 
  select_("FLUX","wavelength") %>% rename(flux = FLUX)
y2 <- as_tibble(read.fitstab(file = "../hw3/shapley_lyman_break_template.fit.txt")) %>% 
  mutate(wavelength = 10^LOGLAM)  %>% 
  select_("FLUX","wavelength") %>% rename(flux = FLUX)
...
system.time(
for(i in files){
  load(file = paste("../hw3/rdata/",i,".Rdata",sep = ""))
...
system.time(
for(i in 1:length(fitslist)){
  test <- as_tibble(read.fitstab(file = paste("../hw3/fits/",fitslist[i],sep = ""))) %>%
    mutate(wavelength = 10^loglam)
